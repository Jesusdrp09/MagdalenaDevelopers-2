...
function obtenerInfo(nombreABuscar){
    let data;
    let arrayUsers;
        ...
        if (snapshot.exists()) {
          ...
          for(let i = 0; i < arrayUsers.length; i++){
            if(nombreABuscar == ""){
                ...
                if((arrayUsers[i].nombreCompleto).includes(nombreABuscar)){
                    flag = true;
                }else{
                    flag = false;
                }
            }
            if(arrayUsers[i].rol != 2 && flag){
...
let nombreBuscar = document.getElementById("inputNombreABuscar");
btnBuscar.onclick = () => {
    $("#contenedorPerfiles").empty();
    obtenerInfo(nombreBuscar.value);
}